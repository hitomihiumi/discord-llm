...
client.once(Events.ClientReady, async (c) => {
  ...
  setInterval(async () => {
    const avgResponseTime =
      metrics.totalQueries > 0 ? Math.round(metrics.totalResponseTime / metrics.totalQueries) : 0;
    const errorRate = metrics.totalQueries > 0 ? metrics.errors / metrics.totalQueries : 0;
    const ragHitRate =
      metrics.ragHits + metrics.ragMisses > 0
...
client.on(Events.MessageCreate, async (message: Message) => {
  ...
  try {
    ...
    if (message.attachments.size > 0) {
      logger.info(`Processing ${message.attachments.size} attachment(s)`);
      fileContent = await fileService.processAttachments(message.attachments);

      if (fileContent) {
        logger.info(`Added file content: ${fileContent.length} chars`);
      }
    }

    const fullQuery = userQuery + fileContent;

    // Security: sanitize input
    if (!securityService.validateInput(fullQuery)) {
    ...
    if (fileContent) {
      logger.info(`With file content: ${fileContent.length} chars`);
    }
    ...
    if (config.logging.logQueries) {
      const queryForLog =
        userQuery +
        (message.attachments.size > 0 ? fileService.formatFileInfo(message.attachments) : "");
    ...
    const prompt = buildPrompt(systemPrompt, ragContext, conversationHistory, fullQuery);
    ...
    conversationManager.addMessage(message.author.id, {
      role: "user",
      content: userQuery + (fileContent ? " [с прикреплённым файлом]" : ""),
    });
    conversationManager.addMessage(message.author.id, {
      role: "assistant",
      content: response,
    ...
    logger.info(`Total response time: ${responseTime}ms (RAG: ${ragTime}ms, LLM: ${llmTime}ms)`);
    ...
    if (config.logging.logErrors) {
      await discordLogger.logError("Failed to process user message", error as Error, {
